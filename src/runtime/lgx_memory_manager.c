...
#define REFILL_THRESHOLD_AGGRESSIVE                                            \
  192 // Refill when cache < 75% full (for hot classes)
#define REFILL_THRESHOLD_MODERATE                                              \
  128 // Refill when cache < 50% full (for warm classes)
#define REFILL_THRESHOLD_CONSERVATIVE                                          \
  64 // Refill when cache < 25% full (for cold classes)
...
typedef struct thread_cache {
  ...
  uint64_t size_class_histogram[NUM_SIZE_CLASSES]; // Total allocations per size
                                                   // class
  ...
  uint32_t transition_matrix[NUM_SIZE_CLASSES]
                            [NUM_SIZE_CLASSES];  // Transition counts
...
static void track_allocation(lgx_memory_manager_t *manager, void *ptr,
                             size_t size,
...
static void batch_refill_cache(lgx_memory_manager_t *manager,
...
static float calculate_size_class_hotness(thread_cache_t *cache,
                                          int size_class);
...
static void prewarm_predicted_size_class(lgx_memory_manager_t *manager,
                                         thread_cache_t *cache);
static float calculate_prediction_confidence(thread_cache_t *cache,
                                             int from_class);
...
lgx_result_t lgx_memory_manager_init(lgx_memory_manager_t **manager,
  ...
  // TEMPORARILY DISABLED: Prewarming causes memory leaks that need
  // investigation The lockfree pool will be populated on-demand during runtime
  // for (int i = 0; i < NUM_SIZE_CLASSES; i++) {
  //     lgx_result_t prewarm_result = lgx_lockfree_pool_prewarm(i, 64);  // 64
  //     blocks per size class if (prewarm_result != LGX_SUCCESS) {
  ...
  size_t pool_size_per_thread =
      16ULL * 1024 * 1024; // 16MB per thread (reduced)
  ...
  if (mgr->use_huge_pages) {
    ...
    if (global_pool) {
      ...
      double tlb_improvement =
          lgx_hugepages_estimate_tlb_improvement(total_pool_size);
  ...
  mgr->tracked_allocations =
      calloc(mgr->tracked_capacity, sizeof(allocation_header_t));
...
/**
 * Shutdown the memory managerToosilan Kartano (Suinula, Kangasala) –
Arable/Horses Size: Very small estate (sole proprietor, ~1 employee) . Area
mainly cereals/pasture. Primary Production: Cereal and fodder crops, with horse
stables (note: Toosilan Eläinlääkintäpalvelut uses same address, but the kartano
farms hay and oats, and offers boarding). Hiring Evidence: A March 2026
Työpaikka (posted 13.3.2026, closing 23.3.2026) on Laura Rekry found a
“Yleishenkilö maatila ja hevosten hoito” role . The ad (in Finnish) implies
urgent hiring of a farm/horse hand. (This listing is still open as of mid-March
2026.) This is direct evidence of an immediate staffing need. Roles Open:
Yleishenkilö (maatalous/hevostalous) – a full-time farm & stable hand . Likely 1
position. Visa/Sponsorship: No mention. Likely expects EU/EEA or Finnish
workers, but posted on TE suggests openness. Contact: Toosilan Kartano – Tel.
+358 40 093 1068 . (As a sole prop., no public contact name; likely the owner
Jessica von Wendt of the veterinary service uses this for hire.) Address:
Toosilanimentie 203, 36120 Suinula (Kangasala) . No official email; website not
found. (Possible LinkedIn: none.) Sources: Official job ad listing on Laura
Rekry ; Finnish business registry data for contacts
...
lgx_result_t lgx_memory_manager_shutdown(lgx_memory_manager_t *manager) {
  ...
  // CRITICAL: Manually clean up ALL thread caches before shutting down the
  // lockfree pool pthread_key_delete() doesn't call destructors for existing
  // threads
  ...
  for (size_t i = 0; i < manager->cache_count; i++) {
    ...
    if (cache) {
      // Return all cached blocks to the lockfree pool (so they can be freed by
      // pool shutdown)
        ...
        for (uint32_t j = 0; j < cache->count[size_class]; j++) {
          if (cache->slots[size_class][j]) {
            ...
            for (int k = 0; k < MAX_THREADS; k++) {
              if (thread_pools[k].base && ptr >= thread_pools[k].base &&
                  ptr < (void *)((char *)thread_pools[k].base +
                                 thread_pools[k].capacity)) {
                from_pool = true;
                break;
              }
            }

            // Free directly - simpler and avoids lockfree pool complexity
            // during shutdown
            if (!from_pool) {
              free(ptr);
            }
      ...
      for (int hot_class = 0; hot_class < HOT_PATH_SIZE_CLASSES; hot_class++) {
        uint32_t allocated_count =
            atomic_load(&cache->hot_path.watermark[hot_class]);
        for (uint32_t j = 0; j < allocated_count; j++) {
          if (cache->hot_path.preallocated_blocks[hot_class][j]) {
            ...
            // Hot path blocks are NOT in the lockfree pool, so free them
            // directly
            if (!from_pool) {
              free(ptr);
            }
  ...
  // Delete pthread key (will call destructors for remaining threads, but
  // they'll just free the cache)
  pthread_key_delete(manager->cache_key);

  // Now shutdown lock-free pool (this will free all blocks that were returned
  // to it)
...
void *lgx_alloc_aligned(size_t size, size_t alignment) {
  ...
  return lgx_memory_manager_alloc_aligned(runtime->memory_manager, size,
                                          alignment, NULL);
}
 ...
 * Free memory
 *
 * This is the unified free function that handles allocations from different
 * allocators:
 ...
 *
 * IMPORTANT: Frame arena check must come first because
 * lgx_heap_is_heap_pointer() reads memory at ptr-sizeof(header), which could
 * segfault for frame arena pointers.
 */
...
void lgx_memory_manager_free(lgx_memory_manager_t *manager, void *ptr) {
  ...
  if (thread_pools[0].base && ptr >= thread_pools[0].base &&
    ...
    // Pool bump allocations are reclaimed as a block at shutdown — no
    // per-pointer free. The deallocation is already counted at function entry
    // (line 712 above), so just return here without double-counting.
    return;
  }
  ...
  if (cache) {
    ...
    for (int hot_class = 0;
         hot_class < HOT_PATH_SIZE_CLASSES && !is_hot_path_block; hot_class++) {
 ...
 */
void *
lgx_memory_manager_alloc_aligned(lgx_memory_manager_t *manager, size_t size,
                                 size_t alignment,
                                 const lgx_allocation_intent_base_t *intent) {
...
static void cache_destructor(void *cache) {
  if (cache) {
    ...
    void *freed_ptrs[NUM_SIZE_CLASSES * THREAD_CACHE_SIZE +
                     HOT_PATH_SIZE_CLASSES * HOT_PATH_CACHE_SIZE];
    int freed_count = 0;

    // Free all cached objects directly (don't return to lockfree pool during
    // shutdown)
    for (int size_class = 0; size_class < NUM_SIZE_CLASSES; size_class++) {
      for (uint32_t i = 0; i < tc->count[size_class]; i++) {
        if (tc->slots[size_class][i]) {
          ...
          for (int j = 0; j < MAX_THREADS; j++) {
            if (thread_pools[j].base && ptr >= thread_pools[j].base &&
                ptr < (void *)((char *)thread_pools[j].base +
                               thread_pools[j].capacity)) {
              from_pool = true;
              break;
            }
          }
    ...
    // Free hot path cache blocks that are still in the cache (not yet
    // allocated)
    for (int hot_class = 0; hot_class < HOT_PATH_SIZE_CLASSES; hot_class++) {
      ...
      // Blocks that were allocated to users should have already been freed by
      // users But during shutdown, we need to free any remaining blocks
      ...
      for (size_t i = 0; i < g_memory_manager->cache_count; i++) {
        if (g_memory_manager->cache_list[i] == tc) {
          ...
          for (size_t j = i; j < g_memory_manager->cache_count - 1; j++) {
            g_memory_manager->cache_list[j] =
                g_memory_manager->cache_list[j + 1];
          }
...
static thread_cache_t *get_thread_cache(lgx_memory_manager_t *manager) {
  thread_cache_t *cache =
      (thread_cache_t *)pthread_getspecific(manager->cache_key);
  if (cache == NULL) {
    // Allocate aligned memory for the thread cache
    if (posix_memalign((void **)&cache, CACHE_LINE_SIZE,
                       sizeof(thread_cache_t)) != 0) {
      return NULL;
    }
    ...
    if (manager->cache_count >= manager->cache_capacity) {
      ...
      if (new_capacity == 0)
        new_capacity = 64;
      thread_cache_t **new_list =
          realloc(manager->cache_list, new_capacity * sizeof(thread_cache_t *));
      if (new_list) {
...
__attribute__((unused)) static void adapt_cache_sizes(thread_cache_t *cache) {
  ...
  for (int i = 0; i < NUM_SIZE_CLASSES; i++) {
    uint64_t total_accesses =
        cache->size_class_hits[i] + cache->size_class_misses[i];
    if (total_accesses < 10)
      continue; // Need some data to adapt
...
__attribute__((unused)) static void adapt_refill_strategy(thread_cache_t *cache,
                                                          int size_class) {
  uint64_t total_accesses =
      cache->size_class_hits[size_class] + cache->size_class_misses[size_class];
  ...
  if (cache->consecutive_misses[size_class] > 5) {
    ...
    cache->refill_batch_size[size_class] =
        (cache->refill_batch_size[size_class] * 3) / 2;
...
calculate_refill_batch_size(thread_cache_t *cache, int size_class) {
...
__attribute__((unused)) static bool should_prewarm_cache(thread_cache_t *cache,
                                                         int size_class) {
  ...
  uint64_t current_time = lgx_time_now_ns();
  uint64_t time_since_refill =
      current_time - cache->last_refill_time[size_class];
  if (time_since_refill > 1000000 &&
      cache->count[size_class] < cache->capacity[size_class] / 2) {
    return true;
  }
...
batch_refill_cache(lgx_memory_manager_t *manager __attribute__((unused)),
                   thread_cache_t *cache, int size_class) {
  ...
  int added = 0;
  for (int i = 0;
       i < popped && cache->count[size_class] < cache->capacity[size_class];
       i++) {
  ...
  if (needed > 0 && cache->count[size_class] < cache->capacity[size_class]) {
    for (int i = 0;
         i < needed && cache->count[size_class] < cache->capacity[size_class];
         i++) {
...
__attribute__((unused)) static void
track_allocation_pattern(thread_cache_t *cache, int size_class) {
...
__attribute__((unused)) static void
prewarm_hot_size_classes(lgx_memory_manager_t *manager __attribute__((unused)),
                         thread_cache_t *cache) {
  for (int i = 0; i < NUM_SIZE_CLASSES; i++) {
    ...
    if (cache->count[i] < target_capacity) {
      ...
      if (still_needed > 0) {
        for (int j = 0;
             j < still_needed && cache->count[i] < cache->capacity[i]; j++) {
...
__attribute__((unused)) static void
update_markov_transition(thread_cache_t *cache, int size_class) {
  // Record transition from last size class to current
  if (cache->last_size_class != 0xFF &&
      cache->last_size_class < NUM_SIZE_CLASSES) {
...
static void recompute_markov_predictions(thread_cache_t *cache) {
  ...
  for (int from_class = 0; from_class < NUM_SIZE_CLASSES; from_class++) {
    ...
    cache->prediction_confidence[from_class] =
        calculate_prediction_confidence(cache, from_class);
  }
...
__attribute__((unused)) static void
prewarm_predicted_size_class(lgx_memory_manager_t *manager
  ...
  uint32_t target_capacity =
      (uint32_t)(cache->capacity[predicted] * (0.30f + confidence * 0.50f));
  ...
  if ((uint32_t)current_count < target_capacity) {
    ...
    for (int i = 0;
         i < popped && cache->count[predicted] < cache->capacity[predicted];
         i++) {
    ...
    if (still_needed > 0 &&
        still_needed <= 32) { // Limit to avoid over-allocation
      for (int i = 0; i < still_needed &&
                      cache->count[predicted] < cache->capacity[predicted];
           i++) {
...
static void prewarm_hot_path_cache(hot_path_cache_t *cache) {
  for (int hot_class = 0; hot_class < HOT_PATH_SIZE_CLASSES; hot_class++) {
    ...
    for (int i = 0; i < HOT_PATH_CACHE_SIZE; i++) {
      // Use malloc directly instead of pool_alloc to avoid thread pool
      // allocation Thread pool allocations can't be individually freed, causing
      // leaks
...
static void *allocate_ultra_fast(lgx_memory_manager_t *manager, size_t size) {
  ...
  if (ptr) {
    ...
    if (slot_idx + 1 < HOT_PATH_CACHE_SIZE) {
      __builtin_prefetch(
          hot_cache->preallocated_blocks[hot_class][slot_idx + 1], 0, 3);
    }
...
sizeof(allocation_header_t));
        ...
        if (pool->slab_count >= pool->slab_capacity) {
            size_t new_capacity = pool->slab_capacity ? pool->slab_capacity * 2
: 8; void** new_slabs = realloc(pool->slabs, new_capacity * sizeof(void*)); if
(!new_slabs) { free(slab); pthread_mutex_unlock(&pool->mutex); return NULL;
            }
        ...
        for (size_t i = 0; i < blocks_per_slab; i++) {
            ...
            // Initialize header
            header->generation = atomic_fetch_add(&manager->generation_counter,
1); header->size = block_size; header->size_class = size_class; header->magic =
0xDEADBEEF; header->allocation_time = 0; // Will be set on allocation
...
/*
static void return_to_pool(lgx_memory_manager_t* manager, void* ptr, int
size_class) { if (size_class < 0 || size_class >= NUM_SIZE_CLASSES) { free(ptr);
        return;
    }
...
static void track_allocation(lgx_memory_manager_t *manager,
                             void *ptr __attribute__((unused)), size_t size,
                             const lgx_allocation_intent_base_t *intent) {
  if (!intent)
    return;
  ...
  if (manager->tracked_count >= manager->tracked_capacity) {
    ...
    allocation_header_t *new_tracked =
        realloc(manager->tracked_allocations,
  ...
  // Add tracking entry
  allocation_header_t *entry =
      &manager->tracked_allocations[manager->tracked_count++];
...
void *lgx_memory_manager_alloc_with_intent_ex(lgx_memory_manager_t *manager,
  ...
  const lgx_allocation_intent_base_t *base_intent =
      (const lgx_allocation_intent_base_t *)intent;
 ...
 */
lgx_result_t lgx_memory_manager_get_stats(lgx_memory_manager_t *manager,
 ...
 */
lgx_result_t lgx_memory_manager_get_usage_stats(lgx_memory_manager_t *manager,
                                                void *ptr,
  ...
  for (size_t i = 0; i < manager->tracked_count; i++) {
    if ((void *)&manager->tracked_allocations[i] == ptr) {
      ...
      usage->actual_lifetime_ms =
          (current_time - entry->allocation_time) / 1000000;
 ...
 */
lgx_result_t lgx_memory_manager_validate_intent(lgx_memory_manager_t *manager,
                                                void *ptr) {
  if (!manager || !ptr) {
    return LGX_ERROR_INVALID_PARAM;
  }
  ...
  for (size_t i = 0; i < manager->tracked_count; i++) {
    if ((void *)&manager->tracked_allocations[i] == ptr) {
      ...
      if (i < manager->tracked_count - 1) {
        manager->tracked_allocations[i] =
            manager->tracked_allocations[manager->tracked_count - 1];
      }
